...
export default function CartDrawer() {
  ...
  const handleCheckout = async () => {
    try {
      // Get Bacteriostatic Water product if bump is checked
      let bacWaterProduct = null;
      if (orderBump) {
        const { data } = await supabase
          ...
          .maybeSingle();
        bacWaterProduct = data;
      }

      // Check for items without WC mapping
      const unmappedItems = items.filter((item) => !item.product?.wc_product_id);
      if (unmappedItems.length > 0) {
        alert(`Some items in your cart are not available for checkout yet: ${unmappedItems.map((i) => i.product.name).join(', ')}. Please remove them or contact support.`);
        return;
      }

      // Build items array
      const allItems = items
        .filter((item) => item.product?.wc_product_id)
        .map((item) => `${item.product.wc_product_id}:${item.quantity}`);

      // Add bump if checked and product exists in WC
      if (orderBump && bacWaterProduct?.wc_product_id) {
        allItems.push(`${bacWaterProduct.wc_product_id}:1`);
      }
      ...
      const checkoutUrl = `https://revivalbiolabs.com/checkout/?empty-cart&add-to-cart=${allItems.join(',')}`;
      closeCart();
